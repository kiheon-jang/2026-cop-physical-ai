...
- **렌더 스모크 4종 PASS (23:30 에이전트, .venv headless)**:
  - `sim_pcb_reset.py`: self-check 4/4 PASS (카메라 top/closeup, 존 0.15~0.3×±0.075, 임계 -0.0015m).
  - `sim_camera_verification.py`: 30 frames both cameras 캡처 (0.76s).
  - `sim_headless_6dof_video.py`: 6관절 스윕 2501 frames → `sim_6dof_animation.mp4` (10.2s).
