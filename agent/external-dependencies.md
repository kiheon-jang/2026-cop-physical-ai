...
### 우선순위 1 — 운영 차단 (사용자 조치 필요)

- [ ] [장기헌] **Gmail 앱 비밀번호 재발급** — 마감 ASAP (2026-09-16 표면화)
  - 증상: 07:00 아침 보고 메일이 **9/12~9/16 5일 연속 SMTP 535 BadCredentials 실패**.
  - 반영 위치: `~/.hermes/.env` 의 `EMAIL_PASSWORD`.
  - 영향 범위: 메일 발송만 실패. 대시보드 데이터 빌드·git pull·시뮬 파이프라인은 정상.
...
- [v] [장기헌] **RS232 야간 측정 300s timeout 상향 여부 결정** — 실측으로 무위험 확정, 결정 불필요 (2026-09-13)
