...
- [ ] [장기헌] **RS232 야간 측정 300s timeout 상향 여부 결정** *(2026-09-11 제기, 9/13 측정 전 결정 필요)*
  - hermes `cop_sim_env.py` 가 드라이버를 timeout=300 으로 동기 호출 → RS232 4-seed(40 rollout) 측정이
    최대 프레임(실패 rollout 1회 10.4s)이면 ≈416s + 모델 로드 > 300s 로 초과 위험.
  - 초과 시 `cop_measured.marker` 미기록 → 매일 재측정 반복(전진 불가). S1 측정은 164s 로 여유였음.
  - hermes 스크립트라 야간 에이전트가 직접 변경 안 함. 상향(예: 900s) 또는 측정 비동기 분리 필요.
  - 상세: `research/simulation/2026-09-11_phase4-w1-rs232-unplug.md` 이슈 4.
