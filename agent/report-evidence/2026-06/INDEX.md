...
## 2026-06-27
- Phase 1 W4: closed-loop 1사이클 **완료/유지** 재확인 — 드라이버 STAGE=완료/유지(50ep, 성공률 0.70/목표 0.90), 마커 정합. 디스크 검증: `episodes_cl` 50ep/3350frame, `epoch_0099` mtime 6/25 02:17, `rollout_summary.json` 70%/43.7mm 정합 (`research/simulation/2026-06-27_closed-loop-cycle1-hold.md`) → 6월 보고서 [2.사전학습] 1사이클 결과 유지 증거.
- 23:30 nightly sim-test (v3.2): 우선순위 4종 **실제 N회 실행** 회귀 통과 — sim_headless_6dof_video 5/5(2501 frame, avg 9.23s, ~1.1GB), sim_camera_verification 5/5(듀얼카메라 30 frame, avg 0.64s, ~0.7GB), sim_pick_place 0/5 태스크(스크립트 정상 실행 / open-loop grasp 0%·min 32.3cm = known 레거시, ~2.9GB), sim_data_collector closed-loop 스모크 5/5(yield 100%, lift 43.6~46.1mm, scratch `data/episodes_smoke`→삭제, 운영 `episodes_cl` 무손상). 환경 mujoco 3.8.0/.venv (`agent/research-log/2026-06-27.md` 우선순위 스크립트 실제 실행 블록) → 6월 보고서 [2.시뮬환경] 환경 안정성 근거.
- closed-loop rollout 70%(7/10) vs open-loop `sim_pick_place.py` 0%(32.3cm) 대조 유지 → 6월 보고서 [2.사전학습] open/closed-loop 대비 증거.

## 2026-06-28
- Phase 1 W4: closed-loop 1사이클 **완료/유지** 재확인 — 드라이버 STAGE=완료/유지(50ep, 성공률 0.70/목표 0.90), 마커 정합. 디스크 검증: `episodes_cl` 50ep/3350frame, `epoch_0099` mtime 6/25 02:17, `rollout_summary.json` 70%/43.7mm 정합 (`research/simulation/2026-06-28_closed-loop-cycle1-hold-verify.md`) → 6월 보고서 [2.사전학습] 1사이클 결과 유지 증거.
- 23:30 nightly sim-test (v3.2): 우선순위 4종 **실제 N회 실행** 회귀 통과 — sim_headless_6dof_video 5/5(2501 frame, avg 9.21s, ~1.1GB), sim_camera_verification 5/5(듀얼카메라 30 frame, avg 0.62s), sim_pick_place 0/5 태스크(스크립트 정상 / open-loop grasp 0%·min 32.3cm = known 레거시, ~2.9GB), sim_data_collector closed-loop 스모크 3/3(yield 100%, lift 43.3~43.8mm, scratch `data/_smoke_collector`→삭제, 운영 `episodes_cl` 무손상). 환경 mujoco 3.8.0/.venv (`agent/research-log/2026-06-28.md` 23:30 회차 블록) → 6월 보고서 [2.시뮬환경] 환경 안정성 근거.
- closed-loop rollout 70%(7/10) vs open-loop `sim_pick_place.py` 0%(32.3cm) 대조 유지 → 6월 보고서 [2.사전학습] open/closed-loop 대비 증거.

## 2026-06-30
- Phase 1 W4 **마지막 날(6/22~6/30 dated 범위 종료)**: closed-loop 1사이클 **완료/유지** 최종 확인 — 드라이버 STAGE=완료/유지(50ep, 성공률 0.70/목표 0.90), 마커 정합. 디스크 검증: `episodes_cl` 50ep/3350frame, `epoch_0099` 335.9MB mtime 6/25 02:17, `rollout_summary.json` 7/10=0.70·median 43.7mm·scene=scene_grasp_pads.xml 정합 (`research/simulation/2026-06-30_closed-loop-cycle1-hold.md`) → 6월 보고서 [마무리] Phase 1 마감·7월 Phase 2 진입 증거.
- 23:30 nightly sim-test (v3.2): 우선순위 4종 실행 회귀 통과 — sim_headless_6dof_video 3/3(2501 frame), sim_camera_verification 3/3(듀얼카메라 30 frame, avg 0.65s), sim_pick_place 0/5 태스크(스크립트 정상 / open-loop grasp 0%·min 32.3cm = known 레거시, ~10.4s), sim_data_collector closed-loop 스모크 2/2(yield 100%, lift 42.4~43.8mm, `/tmp/cop_smoke_0630` 격리, 운영 `episodes_cl` 무손상). 메모리 used 66.3%/avail 5.79GB. 환경 mujoco 3.8.0/.venv (`agent/research-log/2026-06-30.md` 23:30 라이브 블록) → 6월 보고서 [2.시뮬환경] 환경 안정성 근거.
- closed-loop rollout 70%(7/10) vs open-loop `sim_pick_place.py` 0%(32.3cm) 대조 유지 → 6월 보고서 [2.사전학습] open/closed-loop 대비 증거.
