...
- [ ] **5/12**: `mujoco.Renderer`로 RGB 이미지 추출 검증 ✅ 차단 해제 (2026-05-15)
- [ ] **5/13**: 카메라 파라미터 적용 (기본값 fovy=45, 640x480 사용 — 실측값 있으면 교체)
- [ ] **5/14**: 두 카메라 동시 캡처 + 동기화 검증

### W3 (5/15 ~ 5/21) — 실기 ↔ 시뮬 매핑 검증
- [ ] **5/15**: 시뮬 관절 한계 vs 실기 캘리브레이션 비교
- [ ] **5/16**: 시뮬 무게/관성 조정 (기본값 사용 — 실측값 있으면 교체)
