...
- W3 (8/19 ~ 8/25): ACT 학습 + 측정 — **2026-08-06 조기 완주** (`scripts/render_act_rollout_s1.py`)
  ...
  - 🔄 **2026-08-15 — hold 유지 + 무결성 감사 + 렌더 헬스체크**: 드라이버 STAGE=완료/유지(`episodes_s1`
    100ep·0.925, 새 사이클 미트리거) → 재실행 없음. W2 dated 범위(8/12~8/18) 안, 잔여 W2 = omen
    lerobot 로드 스모크 1건뿐(외부 의존) · dated W1~W3 실행가능 항목 전부 `[v]` · 다음 pending =
    W4 omen 핸드오프(외부 의존) → hold. 비파괴 감사: 타겟 마커 `data/episodes_s1`(17B, `.next` 없음)·
    학습 프로세스 없음·`rollout_summary_s1.json` md5 `fbeef25775e5846ba2e3ce887afd1929` 8/8~8/14 대비
    불변(seed42/7/123/2026 = 0.90/0.90/1.00/0.90 평균 0.925)·datasets s1 100ep/7231f·floor·cl 각
    50ep/3350f 불변 → **회귀/오염 0**. 렌더 헬스체크: `sim_pcb_reset.py` self-check **4/4 PASS**
    (S1 트윈 씬 MuJoCo 3.8 headless 정상, 운영 summary md5 불변). [자가치유] 없음. 상세:
    `2026-08-15_phase3-w3-s1-hold-integrity-audit.md`.
  - 🔄 **2026-08-16 — hold 유지 + 무결성 감사 + 렌더 헬스체크**: 드라이버 STAGE=완료/유지(`episodes_s1`
    100ep·0.925, 새 사이클 미트리거) → 재실행 없음. W2 dated 범위(8/12~8/18) 안, 잔여 W2 = omen
    lerobot 로드 스모크 1건뿐(외부 의존) · dated W1~W3 실행가능 항목 전부 `[v]` · 다음 pending =
    W4 omen 핸드오프(외부 의존) → hold. 비파괴 감사: 타겟 마커 `data/episodes_s1`(17B, `.next` 없음)·
    학습 프로세스 없음·`rollout_summary_s1.json` md5 `fbeef25775e5846ba2e3ce887afd1929` 8/8~8/15 대비
    불변(seed42/7/123/2026 = 0.90/0.90/1.00/0.90 평균 0.925)·datasets s1·floor·cl 불변 → **회귀/오염 0**.
    렌더 헬스체크: `sim_pcb_reset.py` self-check **4/4 PASS** (S1 트윈 씬 MuJoCo 3.8 headless 정상,
    운영 summary md5 불변). [자가치유] 없음. 상세: `2026-08-16_phase3-w2-s1-hold-integrity-audit.md`.
  - 🔄 **2026-08-17 — hold 유지 + 무결성 감사 + 렌더 헬스체크**: 드라이버 STAGE=완료/유지(`episodes_s1`
    100ep·0.925, 새 사이클 미트리거) → 재실행 없음. W2 dated 범위(8/12~8/18) 안, 잔여 W2 = omen
    lerobot 로드 스모크 1건뿐(외부 의존) · dated W1~W3 실행가능 항목 전부 `[v]` · 다음 pending =
    W4 omen 핸드오프(외부 의존) → hold. 비파괴 감사: 타겟 마커 `data/episodes_s1`(17B, `.next` 없음)·
    학습 프로세스 없음·`rollout_summary_s1.json` md5 `fbeef25775e5846ba2e3ce887afd1929` 8/8~8/16 대비
    불변(seed42/7/123/2026 = 0.90/0.90/1.00/0.90 평균 0.925)·datasets s1·floor·cl 불변 → **회귀/오염 0**.
    렌더 헬스체크: `sim_pcb_reset.py --self-check` **4/4 PASS** (S1 트윈 씬 MuJoCo 3.8 headless 정상,
    운영 summary md5 불변). [자가치유] 없음. 상세: `2026-08-17_phase3-w2-s1-hold-integrity-audit.md`.
  - 🔄 **2026-08-18 — W2 종료일 hold 유지 + 무결성 감사 + 렌더 헬스체크**: 드라이버 STAGE=완료/유지
    (`episodes_s1` 100ep·0.925, 새 사이클 미트리거) → 재실행 없음. **오늘은 W2 dated 범위(8/12~8/18)
    마지막 날** — dated W1~W3 실행가능 항목 전부 `[v]`, 잔여 W2 = omen lerobot 로드 스모크 1건뿐(외부
    의존) · 다음 pending = W4(8/26~) omen 핸드오프(외부 의존) → hold. 비파괴 감사: 타겟 마커
    `data/episodes_s1`(17B, `.next` 없음)·마커 3자(trained_on=`episodes_s1:1785931493`·
    measured=`episodes_s1:1786060554`) 8/17 값과 바이트 일치·학습 프로세스 없음·`rollout_summary_s1.json`
    md5 `fbeef25775e5846ba2e3ce887afd1929` 8/8~8/17 대비 불변(seed42/7/123/2026 = 0.90/0.90/1.00/0.90
    평균 0.925)·datasets s1·floor·cl 불변 → **회귀/오염 0**. 렌더 헬스체크: `sim_pcb_reset.py --self-check`
    **4/4 PASS** (S1 트윈 씬 MuJoCo 3.8 headless 정상, 렌더 전후 운영 summary md5 불변). [자가치유] 없음.
    상세: `2026-08-18_phase3-w2-s1-hold-integrity-audit.md`.
  - 🔄 **2026-08-19 — W3 dated 시작일 hold 유지 + 무결성 감사 + 렌더 헬스체크**: 드라이버 STAGE=완료/유지
    (`episodes_s1` 100ep·0.925, 새 사이클 미트리거) → 재실행 없음. **오늘은 W3 dated 범위(8/19~8/25)
    시작일** 이나 W3(ACT 학습+측정) 은 8/6 조기 완주 → dated W1~W3 실행가능 항목 전부 `[v]`, 잔여 W2 =
    omen lerobot 로드 스모크 1건뿐(외부 의존) · 다음 pending = W4(8/26~) omen 핸드오프(외부 의존) → hold.
    비파괴 감사: 타겟 마커 `data/episodes_s1`(17B, `.next` 없음)·마커 2종(trained_on=`episodes_s1:1785931493`·
    measured=`episodes_s1:1786060554`) 8/18 값과 바이트 일치·학습 프로세스 없음·`rollout_summary_s1.json`
    md5 `fbeef25775e5846ba2e3ce887afd1929` 8/8~8/18 대비 불변(seed42/7/123/2026 = 0.90/0.90/1.00/0.90
    평균 0.925)·datasets s1 100ep/7231f·floor·cl 각 50ep/3350f 불변 → **회귀/오염 0**. 렌더 헬스체크:
    `sim_pcb_reset.py --self-check` **4/4 PASS** (S1 트윈 씬 MuJoCo 3.8 headless 정상, 렌더 전후 운영
    summary md5 불변). [자가치유] 없음. 상세: `2026-08-19_phase3-w3-s1-hold-integrity-audit.md`.
  - 🔄 **2026-08-20 — W3 dated 2일차 hold 유지 + 무결성 감사**: 드라이버 STAGE=완료/유지
    ...
    감사: 타겟 마커 `data/episodes_s1`(17B, `.next` 없음)·마커 2종(trained_on=`episodes_s1:1785931493`·
    measured=`episodes_s1:1786060554`) 8/19 값과 바이트 일치·학습 프로세스 없음·`rollout_summary_s1.json`
    md5 `fbeef25775e5846ba2e3ce887afd1929` 8/8~8/19 대비 불변(seed42/7/123/2026 = 0.90/0.90/1.00/0.90
    평균 0.925)·datasets s1 100ep/7231f·floor·cl 각 50ep/3350f 불변 → **회귀/오염 0**. [자가치유] 없음.
    상세: `2026-08-20_phase3-w3-hold-audit.md`.
  - 🔄 **2026-08-21 — W3 dated 3일차 hold 유지 + 무결성 감사**: 드라이버 STAGE=완료/유지
    (`episodes_s1` 100ep·0.925, 새 사이클 미트리거) → 재실행 없음. W3 dated 범위(8/19~8/25) 안이나
    W3(ACT 학습+측정) 은 8/6 조기 완주 → dated W1~W3 실행가능 항목 전부 `[v]`, 잔여 W2 = omen lerobot
    로드 스모크 1건뿐(외부 의존) · 다음 pending = W4(8/26~) omen 핸드오프(외부 의존) → hold. 비파괴
  ...
  - 🔄 **2026-08-23 — W3 dated 5일차 hold 유지 + 무결성 감사 + 렌더 4종**: 드라이버 STAGE=완료/유지
    (`episodes_s1` 100ep·0.925, 새 사이클 미트리거) → 재실행 없음. W3 dated 범위(8/19~8/25) 안이나
    W3(ACT 학습+측정) 은 8/6 조기 완주 → dated W1~W3 실행가능 항목 전부 `[v]`, 잔여 W2 = omen lerobot
    로드 스모크 1건뿐(외부 의존) · 다음 pending = W4(8/26~) omen 핸드오프(외부 의존) → hold. 비파괴
    감사: 타겟 마커 `data/episodes_s1`(17B, `.next` 없음)·마커 2자(measured=`episodes_s1:1786060554`·
    trained_on=`episodes_s1:1785931493`) 불변·학습 프로세스 없음·`rollout_summary_s1.json` md5
    `fbeef25775e5846ba2e3ce887afd1929` 8/8~8/22 대비 불변(seed42/7/123/2026 = 0.90/0.90/1.00/0.90
    평균 0.925)·datasets s1 100ep/7231f·floor·cl 각 50ep/3350f 불변 → **회귀/오염 0**. 렌더 4종 PASS
...
- W1: RS232 커넥터 mesh 모델링, 케이블 분리(꽂힌 케이블 빼기) 시뮬 데이터 합성
  ...
  - [v] closed-loop 분리 expert — 접근 → 파지 → 후퇴, 20-seed 성공률 측정 — **완료**: 검증자 새 시드 1000~1019 **15/20=0.75**(합격선 0.75 경계, 1차 시도 0.70), 5000~5019 0.95, 7000~7099 0.85. 존 근단 PCB x<0.19 는 2/19 로 사실상 실패. 실패 렌더 분류 검증 1건 불합격(jam 원인 = 서보 토크 한계 아닌 shoulder 링크 간섭) → 오후 실기 정합 재구축으로 교체(아래)
  - [v] 합성 데이터 100ep 수집(`data/episodes_rs232`, LeRobot v3 · top/closeup) → `logs/cop_dataset_target.next` 예약으로 학습·측정 사이클 전환 — **완료(옛 환경 수집본)**: 100ep/16,024frame, 시도 132, yield 0.758. 실기 정합 환경 재수집본은 100ep/16,093frame, yield 84%. 전환은 `.next` 대신 `logs/cop_dataset_target` 직접 변경(백업 보존) 후 드라이버로 학습 착수 → 오후 실기 정합 환경으로 재수집(아래)
  ...
  - 🔄 **2026-09-11 오후 — 실기 정합 재구축**: 팔 관절 range = 실기 follower 캘리브(wrist_flex ±93.4° 등)·base 충돌·forcerange 2.94·포트 3개·30fps timestep 적용(`sim/assets/so101_real.xml`, RS232 전용), 실패 원인 4개 수정 expert — 홀드아웃 67000~67399 **349/400 = 0.873**(옛 expert 같은 환경 0.305), 명령 100% 실기 한계 안. 근단 x<0.19 36/80(유의 향상 없음, shoulder 기하 상한 ≈37.5%). 흔들어 빼기 미적용(트윈에 효과 기구 없음). 옛 환경 데이터셋·학습 폐기 → 재수집 100ep(yield 84%, 16,093f, 명령·관절값 100% 실기 한계 안) + ACT 42epoch 재학습 착수(20:27). 상세: `2026-09-11_rs232-real-fidelity-expert.md`.
...
- W2: DR 강화 + 학습, 실기 검증
  ...
  - 🔄 **2026-09-16 — 케이블 출구 실기 정합 + 측정 판정 보강**: 실기 영상 기준 케이블을 후드 뒤쪽(커넥터 축)으로 정정(시각 전용 geom, 동작·판정 불변 — 같은 시드 5개 이전/새 씬 5:5 동일). 렌더 관측이 바뀌어 100ep 재수집(시도 119, yield 84%, 16,093f, 명령·관절값 100% 실기 한계 안) + ACT 42epoch 재학습 착수(17:13). **측정 판정**: 목표치(부분성공 50%)·임계(2.95/5.9mm)는 그대로 두고 '임계를 넘는 순간 양 jaw 가 후드에 접촉' 조건만 추가(비핀치 누르고 끌기가 부분성공으로 잡히던 결함 수정 — 수치를 낮추기만 함). 옛 방식 수치는 `*_nopinch_legacy` 로 1사이클 병기.
  - 🔄 **2026-09-16 야간(23:00) — RS232 ACT 재학습 진행 중(STAGE=학습중)**: 낮 세션의 케이블 출구 실기 정합 + 측정 핀치 판정 재수집본(`episodes_rs232` 100ep/16,093f)으로 재학습 전진, pid **48360** alive(state `Us`=세션 분리, 04:04 killer 회피), `train_act.py --epochs 42`→`checkpoints/act_rs232_sim`, 라이브 epoch **7/42** loss 0.0431 정상 수렴. 체크포인트는 아직 직전 완료 run(epoch_0009~0041, Sep 12~13) = 새 run epoch 7 이라 신규 ckpt 0개(첫 저장 epoch 9 미도달, 정상). 마커: trained_on=`episodes_rs232:1789126035`·pending=`episodes_rs232:1789243031`·measured=`episodes_rs232:1789243031`(낮 churn 해소용 현 MODEL_SIG 기록). rollout 산출물 9/15 불변=야간 측정 미실행(설계대로), baseline 부분성공 0.875/완전분리 0.75 무손상. **churn(9/13~15) 종료 확인**(timeout 600/3600 + measured 마커) · **신규 모델 측정 누락 위험 없음**(MODEL_SIG=ckpt mtime 기반 → 재학습 저장 시 자동 재측정). 렌더 헬스 `sim_camera_verification.py` PASS. [자가치유] 없음. 학습 미완이라 851 재측정·854 항목 `[ ]` 유지. 상세: `2026-09-16_rs232-act-retrain-inflight.md`.
  - 🔄 **2026-09-17 야간(23:00) — RS232 ACT 재학습 진행 중(STAGE=학습중, Day 2)**: 어제 착수한 케이블 출구 정정 재학습(pid **48360** alive, state `Us`=세션 분리) 전진, 라이브 epoch **37/42**(36 완료) loss **0.00904**(l1 0.00784·kl 0.000119), from-scratch 곡선 정상(ep7 0.0431→ep37 0.0090). **체크포인트 신선도 직접 검증**: dir mtime 은 Sep 12~13 고정이나 이는 `save_pretrained` 인플레이스 덮어쓰기 특성(결함 아님) — 내부 `epoch_0009/model.safetensors` **Sep 17 00:59**·`epoch_0029/model.safetensors` **Sep 17 17:04** 신선 + 로그 `checkpoint_saved` epoch_0019/0029 기록으로 정정 모델 정상 저장 확증. 잔여 5 epoch(~4h) → **ETA ~03:00~03:30 KST 9/18**(04:04 killer 이전+세션 분리 안전). 마커: trained_on=`episodes_rs232:1789126035`·pending=`episodes_rs232:1789243031`(대기·미승격)·measured=`episodes_rs232:1789243031`. 학습 미완→승격/측정 보류(설계대로)→baseline(`rollout_summary_rs232.json` Sep 15·seed42 0.9/0.7·4-seed 0.875/0.75)·`cop_rollout.log` Sep 15 불변. **신규 모델 측정 누락 위험 없음**(MODEL_SIG=ckpt mtime → 재학습 epoch_0041 덮어쓰기 시 자동 재측정). **이월 해소**: 근단(x<0.19) 실패 분해용 플러그 초기좌표는 측정기가 이미 per-rollout `pcb.x` 로 덤프 중 → 코드 추가 불필요. [자가치유] 없음. 학습 미완이라 851 재측정·854 항목 `[ ]` 유지. 상세: `2026-09-17_rs232-act-retrain-inflight-day2.md`.
  - ✅ **2026-09-18 — 케이블출구 정합+핀치판정 재학습 완주 + 4-seed 측정 완료**: 3일 in-flight 재학습이
    **02:42:32 완주**(`act_rs232_sim/epoch_0041` 새 mtime, 세션 분리로 04:04 killer 이전, train pid 없음)
    → 드라이버 STAGE=측정 4-seed(42/7/123/2026)×10 자동 재측정(23:02~23:06, cpu 83s/seed, 핀치+`*_nopinch_legacy`
    병기). **결과: 신 핀치 판정 부분성공/완전분리 평균 0.600/0.600**(seed42 0.6/0.5·7 0.5/0.6·123 0.7/0.7·2026 0.6/0.6)
    **· legacy(옛 판정) 0.925/0.825**. **완료 기준(Phase 4 = 부분성공 50%) 충족(0.600>0.50)**. 핀치 판정이 헤드라인을
    0.875/0.75→0.600 으로 낮춘 건 하락 아닌 **비핀치 false-positive 제거**(핀치반전 seed42/7 각 1건 = 그 제거분);
    legacy 0.925/0.825 가 옛 baseline 0.875/0.75 보다 높음 → **모델 자체는 정합 재학습으로 소폭 개선**. 실패 롤아웃
  ...
  - [v] RS232 DR 합성 + 재학습 — **완료 2026-09-26**: DR 재학습(`episodes_rs232_dr`→`act_rs232_dr_sim`) 09-26 06:27 완주 → 드라이버 4-seed 측정. **DR-trained 부분성공(pinch) 평균 0.425 / 완전분리 0.325**(seed 42/7/123/2026 = 0.6/0.5/0.5/0.1) · legacy 0.875/0.775. **nominal baseline 0.600 대비 −0.175(하락)** → **DR 축 증강은 RS232 sim 천장 못 올림, 오히려 소폭 하락**(Phase 2 W1 결론 재확증). 하락 원천 = seed2026 배치 붕괴(legacy 0.8 유지 = 비핀치 밀고끌기 증가). 병목 = 배치 커버리지(모방격차). Phase 4 판정(부분성공 50%)은 nominal 0.600 으로 이미 충족 — DR 은 비교 arm, 판정 불변. 상세: `2026-09-26_rs232-dr-trained-4seed-verdict.md`.
    ...
    - 🔄 **2026-09-21 — DR 재학습 트리거 대기 hold + 무결성 감사**: 드라이버 STAGE=완료/유지(`episodes_rs232`
      100ep·0.600) — 새 사이클 조건(`cop_dataset_target`→`episodes_rs232_dr` 전환/마커 삭제) 미세팅으로 nominal
      타겟 유지, 재학습 미트리거. **무결성 전수 감사**: target `episodes_rs232`(`.next`/`.pending` 없음)·trained_on
      `episodes_rs232:1789546321`·measured `episodes_rs232:1789666953`·`rollout_summary_rs232.json` seed42 0.600
      mtime Sep 18 불변·`episodes_rs232` 100ep/16,093f·`episodes_rs232_dr` 100ep/15,337f·학습 프로세스 없음 →
      회귀/오염 0. **RS232 트윈 렌더 헬스**: `sim_rs232_unplug.py` 기본 7/7 PASS + 실기 정합 [8][9][11] PASS
      (보유력 7.2N·핀치 FULL 21/21·대조군 0/21). 재학습은 드라이버 담당(하드룰상 에이전트 직접 실행 금지) → 준비
      완료 표면화만, 블로커 아님. [자가치유] 없음. 학습 미트리거라 866 `[ ]` 유지. 상세:
      `2026-09-21_phase4-w2-rs232-dr-retrain-pending-hold.md`.
    - 🔄 **2026-09-22 — DR 재학습 트리거 대기 hold(3일째) + 무결성 감사**: 드라이버 STAGE=완료/유지(`episodes_rs232`
      100ep·0.600) — 새 사이클 조건(`cop_dataset_target`→`episodes_rs232_dr` 전환/마커 삭제) 미세팅으로 nominal
      타겟 유지, 재학습 미트리거. **무결성 전수 감사**: target `data/episodes_rs232`(`.next`/`.pending` 없음)·trained_on
      `episodes_rs232:1789546321`·measured `episodes_rs232:1789666953`·`rollout_summary_rs232.json` seed42 부분성공
      0.600/legacy 0.900 mtime Sep 18 불변·`episodes_rs232` 100ep/16,093f·`episodes_rs232_dr` 100ep/15,337f·학습
...
- W3: 최종 모델 선정 (ACT vs DP), 1차 통합 모델 미세조정
  ...
  > — `samples/training/test_diffusion_training.py` 는 구 import 경로(`lerobot.common`)를 쓰는 mock
  > 단위테스트이고, `scripts/train_act.py` 는 ACT 하드와이어(policy 인자 없음)다. DP 체크포인트·
  > 학습 로그 0건. (2) DP 적용에는 정규화 설계 결정이 선행돼야 한다 — action 2·5축이 [-1,1] 밖
  > (min −2.674 / max 2.771)인데 `DiffusionConfig.clip_sample_range=1.0` 이라 그대로 붙이면 생성
  > 궤적이 잘린다. (3) DP 학습 벽시계가 미측정이다(스모크 미실행). 최종 모델 선정과 통합 미세조정은
  > 둘 다 DP 비교 결과를 전제로 하므로 함께 이관한다. **9월 보고 내용은 DP 비교에 의존하지 않는다**
  > (2026-09-17 정정으로 RS232 시뮬 실적으로 교체 완료).
  - [ ] *(이후로 이관 — DP 학습 경로 부재, 정규화 설계 선행 필요)* Diffusion Policy 학습 + ACT 대비 비교 (Phase 2 W4 에서 이관)
  - [ ] *(이후로 이관 — DP 비교 결과 전제)* 최종 모델 선정
  - [ ] *(이후로 이관 — 최종 모델 선정 전제)* PCB(S1) + RS232 통합 정책 미세조정
...
**완료 기준**: 시뮬 분리 부분성공 **50%**
  ...
  > **50% = Phase 4(9월, 당시 이름 "RS232 HHT 결선")의 완료 기준**, **40% = Phase 5(10월 통합 시연)의
  > 보고서 목표값**으로 서로 다른 절에 있던 별개 기준이다. `b76cd4c`(2026-08-06) 일정 압축 때 Phase 5 가
  > 폐지되며 두 줄이 병합돼 같은 지표에 두 기준이 인용되는 혼선이 생겼다.
  > **Phase 4 판정은 50% 다.** 10월 시연 보고 목표(PCB 70% / RS232 40%)는 아래 10월 절에서 따로 쓴다.
...
| 9월 | 기능 완성 (DP 비교는 이후로 이관) | Phase 4 (RS232 + 1차 기능 완성) |
