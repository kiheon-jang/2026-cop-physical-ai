...
PHASE_META: list[dict] = [
    ...
    },
    {
        ...
        "outcome": "제어반 RS232 포트에 꽂혀 있는 점검 단말기(HHT) 케이블을 로봇팔이 빼는(분리) 작업 자동화. "
                   "시뮬 4-seed 40회 측정 결과 부분성공 0.600 · 완전분리 0.600 으로 완료 기준(부분성공 50%)을 충족했습니다 "
                   "(파지 확인 포함 판정 · 2026-09-18 측정). 파지 확인 없는 옛 판정으로는 같은 40회가 0.925 / 0.825 입니다. "
                   "모두 시뮬레이션 측정값이며 실물 로봇 검증은 수행되지 않았습니다.",
        "report_label": "기능 완성 (DP 비교는 이후로 이관)",
    },
]
